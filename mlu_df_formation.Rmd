...
```{R}
my_list <- list.files("C:/Users/Marcus A. Dyhr/Documents/github_madyhr/CogCom/Exercise 3 - Sarah and her Child/data", full.names = TRUE)
```
...
```{R}
sarah_data <- map_df(my_list,function(x) read.CLAN.file(x))
```
...
```{R}
data_SGD <- select(sarah_data,Speaker,Gloss,Date)
```
...
```{R}
write_csv(data_SGD,"sarah_tidy.csv")
```
...
```{R}
str(sarah_data$Speaker)
str(sarah_data$Gloss)
str(sarah_data$Date)
```
...
```{R}
sarah_data_mot <- filter(sarah_data,Speaker == "MOT" | Speaker == "CHI")
```
...
```{R}
sarah_data_mot$Gloss_tidy <- str_replace_all(sarah_data_mot$Gloss, pattern = "[:punct:]", replacement = "")

str_replace_all(sarah_data_mot$Gloss)
```
...
```{R} 
sarah_data_mot$Gloss_count <- stri_count_words(sarah_data_mot$Gloss_tidy)
```
...
```{R}
sarah_grp <- group_by(sarah_data_mot,Speaker,Date)
```
...
mlu_df <- summarise(sarah_grp, word_sum = sum(Gloss_count),
                                         utterances = length(Gloss),
...
```{R}
write_csv(mlu_df,"MLU_overview.csv")
```
